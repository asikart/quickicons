...
class AkquickiconsHelper
{
	...
	public static function addSubmenu($vName = '')
	{
		...
		$folders = JFolder::folders(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_akquickicons'.DS.'views');
		
		foreach( $folders as $folder ){
			if( substr($folder, -2) == 'is' || substr($folder, -1) == 's'){
				
				if( $folder == 'images' && (!$canDo->get('core.edit') || !$canDo->get('core.create')) )
					continue ;
	...
	public static function addIncludePath( $path='' )
    {
        ...
        if (!isset($paths)) {
            $paths = array( AKQUICKICONS_ADMIN.DS.'helpers' );
        }
